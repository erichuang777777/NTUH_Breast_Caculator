...
const { buildAgentSystemContext } = require('./_shared/agent-tools');
...
function compactAgentPayload(body) {
  ...
  return {
    ...
    system_context: buildAgentSystemContext(payload.message || '', patient, payload.system_context, { readJson }),
...
async function callOllamaAgent(body) {
  ...
  try {
    ...
    const systemTools = Array.isArray((agentContext.system_context || {}).called_tools) ? agentContext.system_context.called_tools : [];
    const answerTools = Array.isArray(answer.called_tools) ? answer.called_tools : [];
    const calledTools = [...new Set([...systemTools, ...answerTools])];
    const systemCitations = Array.isArray((agentContext.system_context || {}).citations) ? agentContext.system_context.citations : [];
    return {
      ok: true,
      reply,
      tool_id: toolId,
