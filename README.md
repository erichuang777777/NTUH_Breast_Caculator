...
- 後台：`admin.html`，由 `GET /admin` 或 `GET /admin.html` 提供。
- 後端：`web_app.py` 提供 `/api/*`，前台只讀，藥物新增/修改/刪除與公告設定需後台登入。

### 後台登入

```bash
$env:ADMIN_EMAILS="doctor@example.com"
python web_app.py
```

若資料庫尚無任何 admin，從本機 `127.0.0.1` 第一次使用 email 取得代碼會自動建立第一個管理員。登入代碼會印在 server console，本機開發時也會顯示在後台畫面。
...
| 雙藥價系統 | 健保署公告 PDF（公告日 115/03/23；生效日 115/04/01）+ 台大醫院藥價（2024/12/05），每藥可獨立切換健保/自費 |
...
- 健保藥品支付價格：健保署 115/03/23 公告 PDF，115/04/01 生效
- 藥價更新排程：每月 23 日抓取新的健保署公告 PDF；單一藥物更新僅作補查或院內價追蹤
